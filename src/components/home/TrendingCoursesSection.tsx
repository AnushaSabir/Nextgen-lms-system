    ...
    <section className="py-16 sm:py-24 px-4 sm:px-6 bg-[#070b1a]">
      <div className="container mx-auto max-w-6xl">
        <div className="text-center mb-12 sm:mb-16 reveal">
          ...
          <p className="text-base sm:text-lg text-[#50BED9] max-w-xl mx-auto">Handpicked by our experts — the courses everyone is enrolling in right now.</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {TRENDING.map((course, i) => (
            <Link
              ...
              key={course.id}
              className="theme-card card-3d rounded-[2rem] group reveal block overflow-hidden border border-white/10 hover:border-primaryBlue/30 bg-[#101010]/80 border border-white/10 text-white/[0.02] hover:bg-[#50BED9]/5 transition-all duration-500 shadow-lg hover:shadow-[0_20px_45px_rgba(26, 107, 46, 0.1)]"
            ...
            >
              <div className="aspect-video relative overflow-hidden rounded-t-[2rem]">
                ...
                <div className="absolute top-3 left-3 px-3 py-1 rounded-full glass-card text-[9px] font-black text-white uppercase border border-[rgba(240,89,31,0.2)] z-10 backdrop-blur-md">{course.level}</div>
                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all bg-black/30 backdrop-blur-[2px] z-20">
                  <div className="px-5 py-2.5 bg-[#8b5cf6] text-white text-xs font-black rounded-full shadow-lg shadow-primaryBlue/30 transform scale-90 group-hover:scale-100 transition-transform duration-300">View Course</div>
                </div>
              ...
              <div className="p-5 sm:p-6 space-y-2 text-left flex flex-col h-full justify-between">
                <div>
                  <h4 className="text-base sm:text-lg font-black text-white group-hover:text-white transition-colors line-clamp-1 tracking-tight leading-snug">{course.title}</h4>
                  <p className="text-sm text-[#50BED9]/80 line-clamp-2 mt-1">{course.description}</p>
                </div>
                <div className="flex flex-col gap-1 sm:flex-row sm:justify-between sm:items-center text-xs text-[#50BED9] font-medium pt-3 border-t border-white/10 mt-auto">
                  <span className="flex items-center gap-1.5"><course.Icon className="w-3.5 h-3.5 text-white" />{course.level}</span>
        ...
        <div className="text-center mt-10 reveal">
          <Link href="/courses" className="inline-flex items-center gap-1.5 text-white font-black hover:underline underline-offset-8 text-sm sm:text-base">
